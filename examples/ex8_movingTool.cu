...
int main(int argc, char const *argv[])
{
    ...
    std::cout << "Savesteps = " << timestep.saveSteps << "\n";
    ...
    domainHandling::translateBoundaryConditions(BCsH,sizeWalls,sizeWalls+sizeMoving,-0.67f,0.0f,0.12f,true);
    ...
    {
        if(particlesH.u.z[i] > 0.14f)
        {
            NumberOfActiveParticles = i;
            std::cout << i << "\t: z = " << particlesH.u.z[i] << "\n";
            break;
        } 
    }
    ...
    std::ofstream force(output_folder + "/force.csv");
    energy << "Kin.\tPot.\tTot.\n";
    ...
    for(int i = 0; i < numberOfLaunches; i++)
    {
        ...
        if(true)
        {
            domainHandling::translateBoundaryConditions(BCsH,sizeWalls,sizeWalls+sizeMoving,0.00007f,0.0f,0.0f);
            domainHandling::translateBoundaryConditions(BCsD,sizeWalls,sizeWalls+sizeMoving,0.00007f,0.0f,0.0f);
        }
